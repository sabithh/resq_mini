...
import time
from ai_agent import AIAnalyzer
...
class Detector:
    def __init__(self, conf: float = 0.15):
        ...
        self.llm_cache = {}  # { track_id: {"pose": str, "priority": str, "risk_score": float, "last_update": float} }
    ...
    def detect(self, image: np.ndarray, conf: float = None) -> List[Dict]:
        ...
        if pose_results and pose_results[0].boxes is not None:
            ...
            for i, box in enumerate(boxes):
                ...
                # HYBRID AI: Ask Gemini every 10 seconds per tracked victim
                t_id = victims[-1]["id"]
                now = time.time()
                if t_id not in self.llm_cache or (now - self.llm_cache[t_id]["last_update"] > 10.0):
                    try:
                        pad = 30
                        ...
                        if crop.size > 0:
                            print(f"[Hybrid AI] Analyzing victim {t_id} with Gemini...")
                            ai_res = self.ai_analyzer.analyze_scene(crop)
                            if not ai_res.get("error"):
                                llm_pose = ai_res.get("victim_status", pose).split(",")[0][:15] 
                                llm_priority = ai_res.get("urgency_level", "MEDIUM")
                                llm_risk = 0.9 if llm_priority in ["HIGH", "CRITICAL"] else (0.5 if llm_priority == "MEDIUM" else 0.2)
                                if llm_priority == "CRITICAL": llm_priority = "HIGH"
                                self.llm_cache[t_id] = {
                                    "pose": llm_pose.upper(),
                        ...
                        print(f"[Hybrid AI] Error analyzing victim {t_id}: {e}")

                if t_id in self.llm_cache:
                    victims[-1]["pose"] = self.llm_cache[t_id]["pose"]
                    victims[-1]["override_priority"] = self.llm_cache[t_id]["priority"]
                    victims[-1]["override_risk"] = self.llm_cache[t_id]["risk_score"]
